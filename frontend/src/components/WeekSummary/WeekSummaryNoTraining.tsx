...
const nameRenderer = (params: { data?: Student }) => {
  const d = params.data;
  if (!d) return "-";
  return [d.name, d.lastName].filter(Boolean).join(" ");
};

export default function WeekSummaryNoTraining({
  data,
  loading,
  onBack,
  isMobile,
  ...
  if (isMobile) {
    return (
      <div className="space-y-4">
        ...
        <h2 className="text-xl font-semibold text-gray-900">Alunos +7 dias sem treino</h2>
        {data.length === 0 ? (
          <p className="text-muted-foreground">Nenhum aluno nessa situação.</p>
        ) : (
